...
ARG MONGO_VER="8.0.17"
ARG OMADA_VER="6.0.0.25"
ARG OMADA_DOWNLOAD_URL="https://static.tp-link.com/upload/software/2025/202512/20251203/Omada_SDN_Controller_v6.0.0.25_linux_x64_20251120205736.tar.gz"
...
FROM ${OS_BASE} AS build-stage
  ...
  ARG DAEMON_VERSION="1.5.1" 

  RUN apt-get -yq update && \
      apt-get -yq install apt-utils curl ${JAVA_PKG} libcap-dev autoconf make gcc

  RUN set -ex; \
    curl -fsSL -O "https://dlcdn.apache.org/commons/daemon/source/commons-daemon-${DAEMON_VERSION}-src.tar.gz" && \
    curl -fsSL -O "https://downloads.apache.org/commons/daemon/source/commons-daemon-${DAEMON_VERSION}-src.tar.gz.sha512" && \
    # Verify checksum
    sha512sum -c commons-daemon-${DAEMON_VERSION}-src.tar.gz.sha512 && \
    tar xzf "commons-daemon-${DAEMON_VERSION}-src.tar.gz" && \
    cd "commons-daemon-${DAEMON_VERSION}-src/src/native/unix/" && \
    ...
    # Cleanup
    cd / && \
    rm -rf "commons-daemon-${DAEMON_VERSION}-src" "commons-daemon-${DAEMON_VERSION}-src.tar.gz"*
